...
HEADERS = {
    "User-Agent": "hospital-job-watch/0.1 (+https://github.com/nailito/aphp-job-bot)",
...
class ScrapingError(RuntimeError):
    """Raised when a complete HCL snapshot cannot be retrieved safely."""
...
def resolve_term_labels(
    ...
    if missing_ids:
        ...
        try:
            ...
            logger.warning(f"Résolution taxonomy '{taxonomy}' échouée pour IDs {missing_ids} : {e}")
...
def parse_offer(
    ...
    contrat_ids = raw.get(TAXONOMY_MAP["contrats"], []) or raw.get("job_contract_type", []) or []
    ...
    if not contrat_labels and raw.get("job_contract_type"):
        contrat_labels = resolve_term_labels(session, "job_contract_type", raw["job_contract_type"])
    ...
    if starting_ts:
        date_debut = datetime.datetime.fromtimestamp(starting_ts, datetime.UTC).strftime("%Y-%m-%d")
    else:
        date_debut = None
...
def fetch_all_offers_raw(session: requests.Session) -> list[dict]:
    ...
    while True:
        ...
        try:
            ...
            raise ScrapingError(f"Erreur réseau page {page}: {e}") from e

        # An out-of-range page is normal only when the API did not advertise a
        # remaining page. Otherwise accepting it would produce a partial snapshot.
        if resp.status_code in (400, 404):
            if page > 1 and (expected_pages is None or page > expected_pages):
                logger.info(f"Page {page} hors limites (HTTP {resp.status_code}) — fin pagination.")
                break
            raise ScrapingError(f"Page {page} indisponible (HTTP {resp.status_code})")

        resp.raise_for_status()

        if page == 1:
            try:
                expected_pages = int(resp.headers.get("X-WP-TotalPages", ""))
                expected_total = int(resp.headers.get("X-WP-Total", ""))
            except ValueError:
        ...
        if not batch:
            if expected_pages is not None and page <= expected_pages:
                raise ScrapingError(f"Page {page} vide avant la fin attendue")
        ...
        # Fallback for servers that do not return pagination headers.
        if expected_pages is None and len(batch) < PER_PAGE:
            break

        page += 1
        time.sleep(SLEEP_BETWEEN_PAGES)

    if expected_total is not None and len(all_raw) != expected_total:
        raise ScrapingError(
            f"Snapshot HCL incomplet: {len(all_raw)} offres reçues sur {expected_total} attendues"
        )

    if not all_raw:
        raise ScrapingError("Aucune offre HCL reçue; upsert annulé par sécurité")
...
def run_scraper(known_ids: set[int]) -> list[dict]:
    ...
    for raw in raw_offers:
        try:
            ...
            raise ScrapingError("Snapshot HCL incomplet après normalisation") from e
