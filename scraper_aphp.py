    ...
    try:
        if last_message_id is None:
            ...
            requests.post(
                ...
                json={"chat_id": TELEGRAM_CHAT_ID, "message_id": last_message_id, "text": message},
                timeout=10,
            )
...
def fetch_page(page: int, retries: int = 3) -> dict:
    payload = {"facets": {}, "currentPage": page, "onlyCmsJobs": False, "loadOffers": True}
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code == 429:
                ...
                notify(f"⏳ Limite HTTP page {page}; nouvelle tentative dans {wait}s")
...
# SCRAPER
# ─────────────────────────
def scrape_jobs(url=None, max_pages=115, known_ids: set[str] | None = None) -> list[dict]:
    """Return a complete snapshot, preserving legacy IDs already stored in the database."""
    ...
    if total_pages > max_pages:
        raise ScrapingError(
            f"Snapshot AP-HP incomplet: {total_pages} pages requises, limite={max_pages}"
        )
        ...
        try:
            ...
            for o in offers:
                try:
                    ...
                    # Keep a stable identifier regardless of whether the description
                    # contains a human-readable reference.
                    job_id = str(raw_id)
                    legacy_id = f"ID_{raw_id}"
                    if job_id not in known_ids and legacy_id in known_ids:
                        job_id = legacy_id
                    ...
                    skipped_error += 1
                    print(f"[ERROR_JOB] {type(e).__name__}")
                    continue

        except Exception as e:
            notify(f"⚠️ Erreur page {page}: {type(e).__name__}")
            raise
...
"""
    ...
    if skipped_no_id or skipped_error or len(jobs) != total_jobs:
        raise ScrapingError(
            f"Snapshot AP-HP incomplet: {len(jobs)} offres normalisées sur {total_jobs} attendues"
        )
