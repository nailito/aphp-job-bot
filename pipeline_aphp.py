    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                print(f"   ⚠️ DB error (tentative {attempt + 1}/{retries}): {type(e).__name__}")
                time.sleep(delay)
            else:
                raise
...
def get_counts(new_ids: set):
    ...
    def _fn(conn):
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT COUNT(*) FROM jobs WHERE id IN ({placeholders}) AND rejection_category = 'passed_filter_1'",
                list(new_ids),
            )
            ...
            cur.execute(
                f"SELECT COUNT(*) FROM jobs WHERE id IN ({placeholders}) AND score IS NOT NULL",
                list(new_ids),
            )
...
def run_pipeline():
    ...
    try:
        ...
        from database_aphp import get_all_known_ids, init_db, upsert_jobs
        from config import APHP_JOBS_URL

        init_db()

        t0 = time.time()
        jobs = scrape_jobs(APHP_JOBS_URL, max_pages=115, known_ids=get_all_known_ids())
        ...
        if n_new == 0:
            notify("😴 Aucune nouvelle offre — reprise des analyses encore en attente")
        ...
        for i, job in enumerate(new_jobs, 1):
            ...
            if job.get("metier", "") in EXCLUDED_METIERS:
                ...
                mark_rejected(
                    job["id"], "metier_exclu", f"Contrat exclu : {job.get('contrat', '')}"
                )
                n_rej_metier += 1
            elif job.get("filiere", "") in EXCLUDED_FILIERES:
                mark_rejected(
                    job["id"], "metier_exclu", f"Filière exclue : {job.get('filiere', '')}"
                )
                n_rej_metier += 1
        ...
        t0 = time.time()
        filter_stats = execute_with_retry(lambda conn: run_filter(conn))
        ...
        if pending_filter or pending_scoring:
            raise RuntimeError("Analyses incomplètes : des offres restent à traiter")
        ...
        try:
            save_run(n_scraped, n_new, n_removed, 0, 0, 0, f"error: {error_type}", duration)
        except Exception as log_error:
            print(f"Impossible d'enregistrer l'échec : {type(log_error).__name__}")
        notify(f"❌ Échec pipeline AP-HP : {error_type}")
        # Les exceptions DB/API peuvent contenir un profil, un DSN ou un jeton.
        raise RuntimeError(f"Pipeline AP-HP en échec ({error_type})") from None
