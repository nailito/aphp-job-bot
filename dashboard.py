...
def require_dashboard_password() -> None:
    """Block access until the configured dashboard password is supplied."""
    expected = os.getenv("DASHBOARD_PASSWORD", "")
    if not expected:
        st.error("DASHBOARD_PASSWORD doit être défini avant de lancer le dashboard.")
        st.stop()
        ...
        return pd.Series(
            {
                ...
                "score_points_forts": json.dumps(
                    parsed.get("points_forts", []), ensure_ascii=False
                ),
                "score_points_faibles": json.dumps(
                    parsed.get("points_faibles", []), ensure_ascii=False
                ),
            }
        )
...
def load_hcl() -> pd.DataFrame:
    ...
    df["date_publication"] = pd.to_datetime(df["date_publication"], errors="coerce")
    first_seen_naive = (
        df["first_seen"].dt.tz_localize(None)
        if df["first_seen"].dt.tz is not None
        else df["first_seen"]
    )
...
try:
    ...
    st.error(f"Erreur de connexion à la base de données : {type(e).__name__}")
    st.stop()
...
page = st.sidebar.radio("Navigation", pages, index=pages.index(st.session_state.nav))
...
if page == "📊 Tableau de bord":
    ...
    try:
        if source == "APHP":
            runs = pd.read_sql(
                "SELECT * FROM pipeline_runs WHERE source = 'aphp' ORDER BY run_at DESC LIMIT 1",
                conn,
            )
            if not runs.empty:
                last = runs.iloc[0]
                run_at = pd.to_datetime(last["run_at"], utc=True).tz_convert("Europe/Paris")
                date_fmt = run_at.strftime("%d/%m/%Y")
                heure_fmt = run_at.strftime("%Hh%M")
                is_ok = (
                    str(last.get("status", "")).startswith("success")
                    or last.get("status") == "no_new_offers"
                )
                st.info(
                    f"{'✅' if is_ok else '❌'} Dernière actualisation le **{date_fmt}** à **{heure_fmt}** — `{last['status']}`"
                )
        else:
            runs = pd.read_sql(
                "SELECT * FROM pipeline_runs WHERE source = 'hcl' ORDER BY run_at DESC LIMIT 1",
                conn,
            )
            if not runs.empty:
                last = runs.iloc[0]
    ...
    if source == "APHP":
        ...
        if not df_top.empty:
            ...
            with col_card:
                ...
                date_pub = (
                    best["date_publication"].strftime("%d/%m/%Y")
                    if pd.notna(best.get("date_publication"))
                    else "–"
                )
                safe_best = {key: escape(str(value)) for key, value in best.items()}
                st.markdown(
                    f"""
                <div style="background:#f0f9ff;border-left:4px solid #6366f1;padding:16px;border-radius:8px;color:#111">
                    <div style="font-size:1.2rem;font-weight:700">{safe_best["title"]}</div>
                    <div style="color:#444;margin-top:4px">🏥 {safe_best["hopital"]} · 📍 {safe_best["location"]} · 📄 {safe_best["contrat"]} · 📅 {date_pub}</div>
                    <div style="margin-top:8px">🎯 <b>Score : {score_val}/100</b> · Priorité : <b>{safe_best.get("priorite", "–")}</b></div>
                    <div style="color:#555;margin-top:6px;font-size:0.9rem">{safe_best.get("score_raison", "")}</div>
                </div>
                """,
            ...
            with col_btn:
                ...
                st.link_button(
                    "🚀 Postuler →", best["url"], use_container_width=True, type="primary"
                )
        ...
        n_a_postuler = len(
            df_active[
                ...
                & ~df_active["id"].isin(already_applied_ids)
                & (
                    pd.to_datetime(df_active["date_publication"], utc=True, errors="coerce")
                    >= cutoff
                )
            ]
        )
    ...
    # ── HCL : KPIs simples
        ...
        n_reject = len(
            df_active[
                df_active["rejection_category"].isin(["diplome_paramedical", "surqualification"])
            ]
        )
        ...
        n_a_postuler_hcl = len(df_active[df_active["id"].isin(_fb_positifs_hcl)])
    ...
    if source == "APHP":
        ...
        n_rej_ia = len(
            df_active[
                df_active["rejection_category"].isin(
                    ["diplome_paramedical", "surqualification", "profil_inadequat"]
                )
            ]
        )
        n_scored_val = len(df_active[df_active["score"].notna()])
        n_passed_no_score = len(
            df_active[
                (df_active["rejection_category"] == "passed_filter_1") & df_active["score"].isna()
            ]
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    df_d["statut_score"] = df_d.apply(
        lambda r: (
            f"✅ {int(r['score'])}/100 [{r.get('priorite', '–')}]"
            if pd.notna(r.get("score"))
            else (
                "✅ Retenue"
                if r.get("rejection_category") == "passed_filter_1"
                else CATEGORY_LABELS.get(
                    r.get("rejection_category", ""),
                    r.get("rejection_category", "") or "⏳ En attente",
                )
            )
        ),
        axis=1,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption(
        f"{len(df_passed)} offres retenues — {len(df_scorees)} scorées, {len(df_non_scorees)} en attente"
    )
        ...
        if df_scorees.empty:
            ...
            if source == "HCL":
                ...
                if selected_s != "–":
                    ...
                    st.markdown(f"### {row['title']}")
                    st.markdown(
                        f"📍 {row['location']} · 📄 {row['contrat']} · {emoji} **{score_val}/100** · Priorité **{prio}**"
                    )
                    if pd.notna(row.get("score_raison")) and row["score_raison"]:
                        st.info(row["score_raison"])
                    try:
                        ...
                        pass
                    st.link_button(
                        "🔗 Voir l'offre →", row["url"], use_container_width=True, type="primary"
                    )
    ...
    with tab_non_scorees:
        if df_non_scorees.empty:
            ...
            st.info(
                "Active `scoring_enabled = True` dans `pipeline_hcl.py` pour scorer ces offres."
            )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        if source == "APHP":
            runs = pd.read_sql(
                "SELECT * FROM pipeline_runs WHERE source = 'aphp' ORDER BY run_at DESC LIMIT 30",
                conn,
            )
        else:
            runs = pd.read_sql(
                "SELECT * FROM pipeline_runs WHERE source = 'hcl' ORDER BY run_at DESC LIMIT 30",
                conn,
            )
    except Exception as e:
    ...
    if not df_nouvelles.empty:
        ...
        n_r = len(
            df_nouvelles[
                df_nouvelles["rejection_category"].isin(
        ...
        tab1, tab2, tab3 = st.tabs(
            [f"✅ Retenues ({n_p})", f"❌ Rejetées ({n_r})", f"🎯 Scorées ({n_s})"]
        )
        ...
        with tab1:
            ...
            if df_p.empty:
                ...
                for _, row in df_p.iterrows():
                    ...
                    date_pub = (
                        row["date_publication"].strftime("%d/%m/%Y")
                    ...
                    header = (
                        f"{emoji} {score}/100 — **{row['title']}** — {loc}"
                        if score != "–"
                        else f"⚪ **{row['title']}** — {loc}"
                    )
            ...
            if df_r.empty:
                ...
                for _, row in df_r.iterrows():
                    cat = CATEGORY_LABELS.get(
                        row.get("rejection_category", ""), row.get("rejection_category", "")
                    )
    ...
    if source == "APHP":
        ...
        if st.button(
            "▶️ Déclencher le pipeline via GitHub Actions", use_container_width=True, type="primary"
        ):
            import requests

            gh_token = os.getenv("GH_WORKFLOW_TOKEN", "")
            gh_repository = os.getenv("GH_WORKFLOW_REPOSITORY", "")
            if not gh_token or not gh_repository:
                st.error("❌ GH_WORKFLOW_TOKEN et GH_WORKFLOW_REPOSITORY sont requis.")
            else:
                try:
                    resp = requests.post(
                        f"https://api.github.com/repos/{gh_repository}/actions/workflows/daily.yml/dispatches",
                        headers={
                            "Authorization": f"Bearer {gh_token}",
                            "Accept": "application/vnd.github+json",
                        },
                        json={"ref": os.getenv("GH_WORKFLOW_REF", "main")},
                        timeout=15,
                    ...
                    st.error(f"❌ Appel GitHub impossible : {type(error).__name__}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_left:
        ...
        for _, row in df_postuler.iterrows():
            ...
            dec_emoji = (
                "⭐"
                if job_id in {f["job_id"] for f in feedbacks if f["decision"] == "⭐"}
                else "👍"
            )
    ...
    with col_right:
        ...
        date_pub = (
            job["date_publication"].strftime("%d/%m/%Y")
            if pd.notna(job.get("date_publication"))
            else "–"
        )
        c1, c2 = st.columns(2)
        c1.metric("Score", f"{score}/100")
        ...
        st.divider()
        st.link_button(
            "🚀 Postuler sur APHP →", job["url"], use_container_width=True, type="primary"
        )

# ══════════════════════════════════════════════════════════════════════════════
# 📨 MES CANDIDATURES (APHP + HCL)
# ══════════════════════════════════════════════════════════════════════════════

elif page == "📨 Mes candidatures":
    st.title(f"📨 Mes candidatures — {cfg['label']}")
    ...
    if source == "APHP":
        ...
        app_table = "hcl_applications"
        jobs_table = "hcl_jobs"
        id_col = "job_id"

    # ── Créer la table HCL si besoin
    if source == "HCL":
        with get_connection() as _conn:
            with _conn.cursor() as _cur:
                _cur.execute("""
                    CREATE TABLE IF NOT EXISTS hcl_applications (
                        ...
                        job_id           INTEGER NOT NULL REFERENCES hcl_jobs(id),
                        statut           TEXT    NOT NULL DEFAULT '👀 En cours d''examen',
    ...
    with st.expander("➕ Ajouter une candidature", expanded=False):
        if source == "APHP":
            ...
            with get_connection() as _conn:
                already_applied = set(
                    pd.read_sql("SELECT job_id FROM applications", _conn)["job_id"].tolist()
                )
            ...
            with get_connection() as _conn:
                ...
                already_applied = set(
                    pd.read_sql("SELECT job_id FROM hcl_applications", _conn)["job_id"].tolist()
                )
            fb_positifs = {f["job_id"] for f in fb_hcl if f["decision"] in ("⭐", "👍")}
            df_eligible = df_active[
                df_active["id"].isin(fb_positifs) & ~df_active["id"].isin(already_applied)
            ].copy()
            df_eligible["label"] = df_eligible.apply(
                lambda r: f"{r['title']} — {r['location']}", axis=1
            )
            ...
            if st.button("💾 Enregistrer la candidature", use_container_width=True, type="primary"):
                with get_connection() as _conn:
                    with _conn.cursor() as _cur:
                        _cur.execute(
                            f"""
                            INSERT INTO {app_table} (job_id, statut, date_candidature, notes)
                            VALUES (%s, %s, %s, %s)
                        """,
                            (int(job_row["id"]), statut_new, date_cand_new, notes_new),
                        )
                    _conn.commit()
    ...
    with get_connection() as _conn:
        if source == "APHP":
            df_apps = pd.read_sql(
                """
                SELECT a.id AS app_id, a.job_id, a.statut, a.date_candidature,
                       a.notes, a.refus_raison, a.updated_at,
                       j.title, j.hopital, j.location, j.contrat,
                       j.url, j.score, j.date_publication
                FROM applications a
            ...
            df_apps = pd.read_sql(
                ...
                SELECT a.id AS app_id, a.job_id, a.statut, a.date_candidature,
                       a.notes, a.refus_raison, a.updated_at,
                       j.titre AS title, '' AS hopital,
                       j.localisation AS location, j.contrats AS contrat,
                       j.url, j.score, j.date_publication
                FROM hcl_applications a
                JOIN hcl_jobs j ON a.job_id = j.id
                ORDER BY a.date_candidature DESC NULLS LAST
            """,
                _conn,
            )
    ...
    # ── Cartes par candidature
    for _, row in df_apps.iterrows():
        score = int(row["score"]) if pd.notna(row.get("score")) else "–"
        subtitle = f"{row['hopital']} — {row['location']}" if source == "APHP" else row["location"]

        with st.expander(f"{row['statut']} — **{row['title']}** — {subtitle}"):
            date_pub = (
                str(row["date_publication"])[:10] if pd.notna(row.get("date_publication")) else "–"
            )
            c1, c2, c3 = st.columns(3)
            c1.metric("Score", f"{score}/100" if score != "–" else "–")
            c2.metric("Publiée le", date_pub)
            c3.metric(
                "Candidature",
                str(row["date_candidature"])[:10] if pd.notna(row.get("date_candidature")) else "–",
            )
            ...
            with col_statut:
                nouveau_statut = st.selectbox(
                    ...
                    key=f"{source}_statut_{row['app_id']}",
                )
            ...
            if nouveau_statut == "❌ Refusée":
                refus_raison = st.text_input(
                    "Raison du refus",
                    value=refus_raison,
                    key=f"{source}_refus_{row['app_id']}",
                )
            ...
            with col_notes:
                nouvelles_notes = st.text_area(
                    ...
                    value=row["notes"] or "",
                    height=80,
                    key=f"{source}_notes_{row['app_id']}",
                )
            ...
            with col_save:
                if st.button(
                    "💾 Sauvegarder", key=f"{source}_save_{row['app_id']}", use_container_width=True
                ):
                    with get_connection() as _conn:
                        with _conn.cursor() as _cur:
                            _cur.execute(
            ...
            with col_del:
                if st.button(
                    "🗑️ Supprimer", key=f"{source}_del_{row['app_id']}", use_container_width=True
                ):
                    with get_connection() as _conn:
                        with _conn.cursor() as _cur:
                            _cur.execute(f"DELETE FROM {app_table} WHERE id=%s", (row["app_id"],))
                        _conn.commit()
                    st.cache_data.clear()
                    st.rerun()
    ...
    with tab_eval:
        if df_a_evaluer.empty:
            ...
            tri = col_tri.selectbox(
                "Trier par",
                [
                    "Score (meilleur en premier)",
                    "Score (moins bon en premier)",
                    "Date de publication",
                ],
                key="tri_eval",
            )
            ...
            if tri == "Score (meilleur en premier)":
                df_a_evaluer = df_a_evaluer.sort_values(
                    "score_num", ascending=False, na_position="last"
                ).reset_index(drop=True)
            elif tri == "Score (moins bon en premier)":
                df_a_evaluer = df_a_evaluer.sort_values(
                    "score_num", ascending=True, na_position="last"
                ).reset_index(drop=True)
            else:
                df_a_evaluer = df_a_evaluer.sort_values(
                    "date_publication", ascending=False
                ).reset_index(drop=True)
            ...
            for idx, row in df_a_evaluer.iterrows():
                ...
                with st.expander(f"{score_label}**{row['title']}** — {row['hopital']}"):
                    ...
                    c1, c2, c3 = st.columns(3)
                    c1.metric(
                        "Score", f"{int(row['score'])}/100" if pd.notna(row.get("score")) else "–"
                    )
...
""",
                                ...
                                st.session_state[f"resume_text_{job_id}"] = response.choices[
                                    0
                    ...
                    btn_top = c_a.button(
                        "⭐ Excellent", key=f"top_{job_key}", use_container_width=True
                    )
                    btn_oui = c_b.button(
                        "👍 Intéressant", key=f"oui_{job_key}", use_container_width=True
                    )
                    btn_non = c_c.button(
                        "👎 Pas intéressant", key=f"non_{job_key}", use_container_width=True
                    )

                    if btn_top:
                        st.session_state[f"dec_{job_id}"] = "⭐"
                    if btn_oui:
                        st.session_state[f"dec_{job_id}"] = "👍"
                    if btn_non:
                        st.session_state[f"dec_{job_id}"] = "👎"
                    ...
                    if f"dec_{job_id}" in st.session_state:
                        ...
                        commentaire = st.text_area(
                            "Ton feedback :", key=f"comment_{job_key}", height=80
                        )
                        if st.button(
                            "💾 Sauvegarder", key=f"save_{job_key}", use_container_width=True
                        ):
        ...
        if df_deja_evalues.empty:
            ...
            for _, row in df_deja_evalues.iterrows():
                ...
                with st.expander(f"{f.get('decision', '?')} **{row['title']}** — {row['hopital']}"):
                    ...
                    with c2:
                        if st.button(
                            "🗑️ Supprimer", key=f"del_{row['id']}", use_container_width=True
                        ):
    ...
    if df_nf.empty:
        ...
        for _, row in df_nf.iterrows():
            ...
            header = (
                f"🆕 {score}/100 — **{row['title']}** — {row['hopital']}"
                if score
                else f"🆕 **{row['title']}** — {row['hopital']}"
            )
    ...
    if df_removed.empty:
        ...
        for _, row in df_removed.iterrows():
            ...
            eval_badge = (
                f"{feedback_map[row['id']]['decision']} Évaluée" if evaluated else "⬜ Non évaluée"
            )

            with st.expander(
                f"🗑️ **{row['title']}** — {row['hopital']} | {ia_badge} | {eval_badge}"
            ):
                c1, c2 = st.columns(2)
                ...
                if evaluated:
                    ...
                    st.markdown(
                        f"**Ton évaluation :** {f['decision']} — {f.get('commentaire', '–')}"
                    )
                st.link_button(
                    "🔗 Voir l'offre →", row["url"], use_container_width=True, type="primary"
                )
    ...
    try:
        ...
        df_deja_evalues = df_active[df_active["id"].isin(fb_existants)].copy()
        ...
        if df_a_evaluer.empty:
            ...
            for idx, row in df_a_evaluer.iterrows():
                ...
                with st.expander(header):
                    ...
                    if btn_top:
                        st.session_state[f"dec_hcl_{job_id}"] = "⭐"
                    if btn_oui:
                        st.session_state[f"dec_hcl_{job_id}"] = "👍"
                    if btn_non:
                        st.session_state[f"dec_hcl_{job_id}"] = "👎"
        ...
        if df_deja_evalues.empty:
            ...
            with get_connection() as _conn:
                fb_full = {
                    f["job_id"]: f
                    for f in __import__(
                        "database_hcl", fromlist=["get_feedbacks_hcl"]
                    ).get_feedbacks_hcl(_conn)
                }
            ...
            for _, row in df_deja_evalues.iterrows():
                ...
                with st.expander(f"{dec} **{row['title']}** — {row['location']}"):
                    ...
                    with c_del:
                        if st.button(
                            "🗑️ Supprimer", key=f"del_hcl_{row['id']}", use_container_width=True
                        ):
                            with get_connection() as _conn:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    df_postuler = df_active[df_active["id"].isin(fb_positifs)].copy()
    ...
    with col_right:
        ...
        emoji = (
            "🟢"
            if isinstance(score_val, int) and score_val >= 80
            else "🟡"
            if isinstance(score_val, int) and score_val >= 60
            else "🔴"
        )
        ...
        st.divider()
        st.link_button(
            "🚀 Postuler sur HCL →", job["url"], use_container_width=True, type="primary"
        )
