"""
database_hcl.py
Couche d'accès PostgreSQL pour les offres HCL (table hcl_jobs).
Adaptée à la structure JetEngine/HCL.
"""
...
def get_active_offers(conn) -> list[dict]:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute("SELECT * FROM hcl_jobs WHERE status = 'active' ORDER BY first_seen_at DESC")
...
def delete_old_offers(conn, months: int = 6) -> int:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            WHERE date_modification < NOW() - (%s * INTERVAL '1 month')
        """,
            (months,),
        )
