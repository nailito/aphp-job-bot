...
from database_aphp import get_offers_to_filter, update_ai_filter
...
def run_filter(conn, limit: int = None) -> dict:
    ...
    for job in tqdm(jobs, desc="Filtre APHP"):
        ...
        try:
            ...
            if result:
                cat, reason = result
                update_ai_filter(conn, job_id, "reject", reason, category=cat)
                stats["rejected"] += 1
            ...
            if reason:
                ...
                stats["auto_passed"] += 1
                logger.debug("Auto-pass métier job=%s", job_id)
                continue
                ...
                try:
                    ...
                    if decision == "reject":
                        update_ai_filter(
                            ...
                            "reject",
                            raison,
                            category=categorie or "profil_inadequat",
                        )
                        stats["rejected"] += 1
                        stats["ai_rejected"] += 1
...
⏳ {stats["skipped"]} laissées en attente
