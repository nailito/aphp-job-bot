...
def load_feedbacks() -> list[dict]:
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            rows = cur.fetchall()
    return [
        {"decision": r[0], "commentaire": r[1], "title": r[2], "metier": r[3], "filiere": r[4]}
        for r in rows
    ]
...
def build_feedback_examples(feedbacks: list[dict]) -> str:
    if not feedbacks:
        ...
        lines = []
        for f in feedbacks:
            decision_label = {"⭐": "EXCELLENT", "👍": "INTÉRESSANT", "👎": "PAS INTÉRESSANT"}.get(
    ...
    if refus:
        ...
        for r in refus:
            lines.append(
                f"- REFUSÉ PAR RECRUTEUR : '{r['title']}' ({r['metier']}) → \"{r['refus_raison']}\""
            )
...
def run_scorer(limit: int = None) -> dict:
    if not GROQ_API_KEY:
    ...
    for i, job in enumerate(jobs, 1):
        ...
        for attempt in range(5):
            try:
                ...
                if not 0 <= score <= 100 or priorite not in {"P1", "P2", "P3"}:
                    raise ValueError("Score ou priorité LLM invalide")
                ...
                scored += 1
                print(f"    🎯 {priorite} — {score}/100")
                if score < 50:
                    print("    ❌ Rejeté automatiquement (score < 50)")
                ...
                err = str(e)
                print(f"    ⚠️ Échec scoring : {type(e).__name__}")
                if "429" in err or "rate_limit" in err:
                    if "per day" in err or "TPD" in err:
                        print("    🛑 Limite journalière atteinte")
                        print(f"    💾 {scored} offres scorées jusqu'ici")
                        return {"scored": scored, "errors": errors, "skipped": len(jobs) - i + 1}
                    else:
                        print("    ⏳ Rate limit/minute, pause 60s...")
                        time.sleep(60)
                else:
                    break
    ...
    return {"scored": scored, "errors": errors, "skipped": 0}
